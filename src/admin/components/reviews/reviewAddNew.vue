			...
			.reviews-form__main-content
				.reviews-form__row
					label.reviews-form__block-name Имя автора
						input(type="text" v-model="review.author").reviews-form__input-name
					label.reviews-form__block-profession Титул автора
						input(type="text" v-model="review.occ").reviews-form__input-profession
				label.reviews-form__block-text Отзыв
					textarea(
						v-model="review.text"
					).reviews-form__text
				.reviews-form__controls
					button(type="button" title="Отмена" @click="cancelAddingReview").btn-main.btn-main--cancel Отмена
					button(type="button" title="Сохранить" @click="addNewReview").btn-main сохранить
</template>
...
export default {
	...
	methods: {
		...mapActions('reviews',["addNew"]),
		cancelAddingReview() {
			this.$emit("cancelAddingReview");
		},
		addPreviewFile(event) {
			this.review.photo = event.target.files[0];
			this.url = URL.createObjectURL(this.review.photo);
			this.background = `background: url(${this.url}) center/cover no-repeat`;
		},
		async addNewReview() {
			try {
				const reviewFormData = new FormData();
				reviewFormData.append('photo', this.review.photo);
				reviewFormData.append('author', this.review.author);
